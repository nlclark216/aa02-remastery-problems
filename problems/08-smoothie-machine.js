...
function smoothieMachine(...list1) {
  let smoothie = "I'm having a smoothie with"
  let smoothArr = smoothie.split(' ')
  function moreIngredients(...list2) {
    ingredients = list1.concat(list2);
    for (let i = 0; i < ingredients.length; i++) {
      let ele = ingredients[i];
      smoothArr.push(ele)
      smoothArr.push('and')
    }
    let slice = smoothArr.slice()
    slice.pop()
    return slice.join(' ')
  }
